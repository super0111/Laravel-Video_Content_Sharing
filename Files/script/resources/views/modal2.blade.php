        ...
        <div class="col-lg-7 p-0">
            ...
            <!-- Indicators -->
                ...
                <div class="carousel-inner">
                    ...
                    foreach($videos as $v) {                    
                    ?>
                    <div class="carousel-item <?php if($v->id == $video->id) {echo "active";}?>">
                        <div class="video-section" onclick="play(<?=$v->id?>)">
                            <video id='singlevideo_<?=$v->id?>' >
            ...
            <div class="modal-right-section">
                        ...
                        <div class="single-comment">
                            ...
                            <span> 
                                ...
                                <div class="comment-info">
                                    <span>{{ $comment->created_at->isoFormat('Do') }} 
                                        <span id="comment_like_count{{ $comment->id }}" class="likes"> 
                                            {{ $comment->favourite_to_user->count() }}likes
                                        </span>
                                        <a href="javascript:void(0)" onclick="reply('{{ $comment->id }}','{{ $comment->user->username }}','{{ $comment->user->id }}')">{{ __('Reply') }}</a>
                                    </span>
                                </div>
                            </span>
                ...
                <div class="modal-video-post-action">
                    <div class="modal-main-action">
                        @if(Auth::check())
                        <a  href="javascript:void(0)"onclick="like('{{ $video->id }}')">
                            <i id="like" class="{{ !Auth::User()->favourite_videos->where('pivot.video_id',$video->id)->count() == 0 ? 'fas fa-heart' : 'far fa-heart' }}"></i>
                        </a>
                        @else
                        <a href="{{ route('login') }}" class="pjax" onclick="profileshow()"><i id="like" class="far fa-heart"></i></a>
                        @endif
