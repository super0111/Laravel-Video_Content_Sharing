(function($) {
    ...
    /*------------------------------
        ...
        ------------------------------------*/
    $(document).on('pjax:complete', function(){
        $('#submit_post2').on('submit',function(e){
            e.preventDefault();
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            ...
            let url = $('#post_url2').val();
            let form = document.getElementById('submit_post2');
            let formdata = new FormData(form);
            formdata.append('video_file',$('#video_path').val());
            $.ajax({
                url: this.action,
...
function commentPopUp(slug)
{
    var url = $("#commentPopuUp_url").val();
    $.ajax({
        url: url,
        data: { slug: slug },
        type: "GET",
        ...
        success: function(response) {
            ...
            $('.loading').addClass('d-none');
            $('.bg-modal-comments').removeClass('d-none');
            $('.modal-content-area-comments').html(response);
            ellipsis(slug);
            ads(slug);
        }
    });
...
/*-------------------
    ...
    -------------------------*/
function play(id) {
    var myVideo = document.getElementById("singlevideo_"+id);
    if (myVideo.paused) {
        myVideo.play();
...
function full_screen_play (video_id) {
    ...
    var elem = document.getElementById("full_screen_play_" + video_id);
    if (elem.paused) {
        elem.play();
...
{
    var data = $('#videoSearch').val();
    var url = $('#videoSearch_url').val();
    ...
    $.ajax({
        ...
        dataType: "HTML",
        success: function(response) {
            $('.video-panel').html(response);
        }
    });
}

function videoSearch_cart()
{
    var data = $('#videoSearch_cart').val();
    var url = $('#videoSearch_url_cart').val();
    ...
    $.ajax({
        ...
        dataType: "HTML",
        success: function(response) {
            $('.video-panel_cart').html(response);
        }
    });
}

function video_update(event) {
    var url = URL.createObjectURL(event.target.files[0]);
    console.log(url)
    $('#current_video_src').attr('src', url)
    var video = document.getElementById('current_video');

    video.load();
    video.play();
    // var image = document.getElementById('output');
    // clearFileInput(document.getElementById("current_video_src"));
}
...
function cartegory(id) {
    console.log(id);
    var url = $("#cartegory_url").val();
    console.log(url);
    $.ajax({
        url: url,
        data: { id: id },
        ...
        success: function(response) {
            console.log(response)
            if(response === "addCart") {
                $('#cartegory').attr('class','fas fa-cart-plus');
                // $('#cartegory').removeClass('fas fa-shopping-cart').addClass('fas fa-cart-plus');
            }
            else if(response === "disCart") {
                $('#cartegory').attr('class','fas fa-shopping-cart');
            }
        }
    });
}
...
function approve(id) {
    ...
    $.ajax({
        ...
        type: "GET",
        beforeSend: function() {
            if ($("#approve_"+id).hasClass("fas fa-times")) {
                $('#approve_text_'+id).text("Approve");
                $('#approve_'+id).attr('class','fas fa-check');
            }else{
                $('#approve_text_'+id).text("UnApprove");
                $('#approve_'+id).attr('class','fas fa-times');
            }
        },
        success: function(response) {
            console.log(response)
